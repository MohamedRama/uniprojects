...
public class Main {
	...
	private static void testConsistency() throws Exception {
		...
		CyclicBarrier barrier = new CyclicBarrier(numThreads+1);  // 5
		ConsistencyWriterThreads[] writerThreads = new ConsistencyWriterThreads[numThreads];
		ConsistencyReaderThread readerThread = new ConsistencyReaderThread(db, barrier);
		readerThread.start();
		for(int threadId=1; threadId < numThreads; threadId++) {  // 3
			writerThreads[threadId] = new ConsistencyWriterThreads(db, barrier, threadId);
			writerThreads[threadId].start();
		}
		barrier.await();
		barrier.await();
		barrier.await();
